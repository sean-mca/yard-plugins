//! EMR plugin handler implementing the `PluginHandler` trait.
//!
...
/// AWS EMR provider plugin handler.
///
...
impl EmrHandler {
    /// Create a new `EmrHandler` with an embedded single-threaded tokio runtime.
...
impl PluginHandler for EmrHandler {
    fn name(&self) -> &str {
        "yard-plugin-emr"
    }
