//! Integration tests for the EMR plugin stdio protocol.
//!
//! Each test spawns the `yard-plugin-emr` binary via `assert_cmd`,
...
fn handshake_contains_plugin_identity() {
    ...
    let output = Command::cargo_bin("yard-plugin-emr")
        .unwrap()
        .write_stdin(format!("{}\n", request))
        .output()
        .unwrap();
    ...
    assert_eq!(handshake["name"], "yard-plugin-emr");
